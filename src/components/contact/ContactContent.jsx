...
const ContactContent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://localhost/north-roadways-service/backend/contact.php', {
                method: 'POST',
                body: formData
            });
            ...
            // alert("Failed to connect to the server. Please check your network.");
             // For static demo purposes, still show success if it fails to connect to PHP
             setIsSubmitted(true);
             setTimeout(() => setIsSubmitted(false), 5000);
        } finally {
            setIsLoading(false);
                                ...
                                >
                                    <form className="space-y-8" onSubmit={handleSubmit}>
                                        <div className="space-y-6">
                                            ...
                                            <div className="space-y-3">
                                                <label className="text-sm font-bold text-heading/70 ml-2 uppercase tracking-widest">Mobile Number *</label>
                                                <input
                                                    type="tel"
                                                    required
                                                    name="mobileNumber"
                                        ...
                                        <div className="pt-4 flex items-center gap-6 flex-wrap md:flex-nowrap">
                                            <button
                                                type="submit"
                                                disabled={isLoading}
                                                className="w-full md:w-auto px-12 py-5 bg-primary text-white font-black text-sm tracking-[0.2em] rounded-2xl hover:bg-primary-hover shadow-xl shadow-primary/30 transition-all active:scale-95 flex items-center justify-center gap-4 cursor-pointer group uppercase disabled:opacity-70 disabled:cursor-not-allowed"
                                            >
                                                {isLoading ? (
